...
class Encoder(nn.Module):
    def __init__(self, embed_model, hidden_dim, span_size, dropout, bert_path=None):
    ...
    def forward(self, inputs, input_lengths, span_length, trees, words):
        ...
        span_output.masked_fill_(span_mask.unsqueeze(-1) == 1, 0)
        batch_index = torch.arange(span_length.size(0))
        if use_cuda:
            batch_index = batch_index.cuda()
        span_hidden = span_output[batch_index, span_length - 1]
